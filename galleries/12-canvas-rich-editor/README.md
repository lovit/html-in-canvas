...
- 한글을 입력해 보자. 조합 중인 글자가 캔버스에 어떻게 나오는지 본다. 툴바 오른쪽에 조합 상태가 표시된다
- 글자를 드래그해 선택해 보자. 선택 영역이 캔버스에 그려진다
- "효과 모두 끄기" 를 누르고 원본 DOM 과 비교해 보자
- `destination-over` 를 `source-over` 로 바꿔 보자. 처음 제가 만들었던 뭉개진 화면이 나온다
- 외곽선 두께를 8 로 올려 보자. 글자 사이가 메워진다
- `spellcheck="false"` 를 지우고 오타를 내 보자. 09 에서 본 것처럼 밑줄은 캔버스에 안 그려진다
