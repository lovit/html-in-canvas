...
카드 내용이 안 바뀌면 업로드는 한 번도 일어나지 않고, 셰이더 효과는 60fps 로 계속 돈다. 비싼 일과 싼 일이 자연스럽게 나뉜다.
...
- `gl.RGBA8` 을 `gl.SRGB8_ALPHA8` 로 바꿔 보자. 색이 밝아진다
- 텍스처 업로드를 `paint` 밖으로 옮겨 보자. `No cached paint record` 를 직접 만난다
- `TEXTURE_MIN_FILTER` 를 `NEAREST` 로 바꾸고 볼록 강도를 올려 보자. 글자가 계단처럼 깨진다
