...
![문서 열두 장이 격자로 놓인 작업판. 측정값에 "화면에 걸친 패널 12/12, 이번 paint 에서 그린 것 2장, changedElements 6, 10" 이 표시돼 있다](screenshot.png)
...
브라우저에서 `galleries/18-document-workspace/` 를 연다. 캔버스를 끌면 움직이고 휠을 굴리면 확대·축소된다.
...
130% 로 확대한 상태에서 패널 6 의 체크박스를 실제 마우스 이벤트로 눌러 봤다.

```text
패널 6 체크박스 자리: 388, 537 → IFRAME(패널 6)
누르기 전 checked = false
누른 뒤   checked = true
```
