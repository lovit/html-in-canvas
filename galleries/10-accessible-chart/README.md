...
`10-accessible-chart/` 로 들어가서 Tab 키를 눌러 보자. 막대에 포커스 테두리가 그려지고 툴팁이 뜬다. 스크린리더로도 읽히도록 만들었는데, 그 부분은 아래 "확인하지 못한 것" 을 보자.
...
### 5. 여기서는 부분 갱신을 쓰지 않는다

04 에서 배운 `changedElements` 를 이 예제는 쓰지 않는다. `paint()` 가 매번 `ctx.reset()` 후 격자·막대·자식을 전부 다시 그린다.

이유는 이 화면이 대부분의 시간에 **전부 바뀌기** 때문이다. 데이터를 바꾸면 막대 여섯 개가 동시에 움직이고 축 눈금까지 다시 잡힌다. 그런 화면에서 부분 갱신을 하면 장부만 늘고 얻는 것이 없다. 04 의 카드 격자처럼 일부만 드문드문 바뀔 때 쓰는 기법이다.

호버와 포커스만 바뀌는 정지 상태에서는 부분 갱신이 이득일 수 있다. 직접 해 볼 만한 과제로 남겨 둔다.

### 6. 애니메이션은 프레임마다 requestPaint
...
- `changedElements` 로 바뀐 것만 다시 그리기 ([04](../04-paint-event/)에서)
...
VoiceOver 로 실제로 읽어 보는 것은 자동으로 검증할 수 없어 아직 확인하지 못했다. `aria-label` 과 대체 표 `<table>` 을 갖췄으니 읽힐 것으로 보지만, 확인한 사실이 아니라 설계 의도다. 직접 켜서 보시고 결과가 다르면 알려 주기 바란다. 같은 이유로 [03. 인터랙티브 폼](../03-interactive-form/)의 스크린리더 항목도 미확인이다.

## 다음 예제

[11. 살아 있는 문서를 3D 공간에](../11-live-document-3d/) — 여기부터 고급편이다. 문서 하나를 통째로 GPU 텍스처로 올린다.
