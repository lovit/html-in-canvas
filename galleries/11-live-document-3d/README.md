...
> 이 문서의 측정값은 Chrome 151.0.7922.138 (macOS) 에서 2026-08-22 에 잰 것이고, 2026-08-23 에 151.0.7922.172 에서 다시 확인했다. 표준화 전 API 라 다음 버전에서 달라질 수 있다.
...
### 1. iframe 도 그냥 자식이다 (단, 스크롤이 생기면 안 된다)

```html
<canvas id="stage" width="760" height="540">
  <iframe
    id="front"
    src="src/page-front.html"
    title="앞장 문서"
    width="360"
    height="480"
    scrolling="no"
...
`scrolling="no"` 는 장식이 아니다. 자식 문서가 프레임보다 길어서 스크롤이 생기면 Chrome 151.0.7922.172 에서는 **스크롤바만 그려지고 문서가 통째로 빠진다.** 예외도 나지 않고 그냥 빈 그림이 된다. 재 본 값과 우회 방법은 [알려진 문제](../../docs/known-issues.md)에 적어 두었다. 어차피 캔버스에 올린 문서는 스크롤할 방법이 없으니, 프레임 크기에 맞춰 두고 넘치면 잘라 내는 편이 맞다.
...
| 종이가 새까맣다                | 자식 문서에 스크롤이 생겼다. `scrolling="no"` 를 보라 |
